#!/usr/bin/python3
from discord import PermissionOverwrite, Intents
...
async def on_voice_state_update(user, vc_before, vc_after):
    ...
    if vc_before.channel is not None:
    ...
    if vc_after.channel is not None:
        ...
        if channel is None:
            ...
            channel = await vc_after.channel.category.create_text_channel(name=vc_txt_after, overwrites=overwrites)
        else:
